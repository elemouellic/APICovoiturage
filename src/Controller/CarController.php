...
public function insertCar(Request $request, EntityManagerInterface $em): JsonResponse
{
    ...
    try {
        ...
        $student = $em->getRepository(Student::class)->findOneBy(['id' => $data['student']]);

        // Check if the student is already possessing a car
        if ($student->getPossess() !== null) {
            throw new HttpException(400, "The student is already possessing a car");
        }
...
return $this->json([
    ...
    'student' => $student->getPossess() ? $student->getPossess()->getId() : null,
]);
}
    /**
     * Delete a car
    ...
    public function deleteCar(int $id, EntityManagerInterface $em): JsonResponse
    {
        try {
            ...
            $student = $em->getRepository(Student::class)->findOneBy(['possess' => $car]);
